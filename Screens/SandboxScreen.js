...
export default function SandboxScreen({ route, navigation }) {
	...
	useEffect(() => {
		...
		if (components.length === 0 && data && data.startItems) {
			const componentNames = data.startItems.map((item) =>
				libraryComponents.find((component) => component.name === item)
			);
				...
				{libraryComponents.map((component, index) => {
					return (
						<CreateComponent
							...
							setComponentsToCreate={setComponentsToCreate}
						/>
					);
				})}
				<Text
		...
		<GestureHandlerRootView style={{ flex: 1 }}>
			<SandboxContext.Provider value={{ isDragging }}>
				<SideMenu
					menu={menu}
					isOpen={sidebarOpen}
					onChange={(open) => setSidebarOpen(open)}
					menuPosition="right"
				>
						...
						{components.map((component, index) => {
							if (component.name === "Sladd") {
								return React.cloneElement(component.component, {
									...
									startY: 25 + components.length * 50,
									channel1: component.channel1,
									channel2: component.channel2,
								});
							}
	...
	const checkConnection = (components, index) => {
		...
		if (dragComponent.name === "Sladd") {
			...
			components.forEach((component, i) => {
				if (component.name !== "Sladd") {
					// Reset channels
					components[i] = resetComponentChannels(
						component,
						dragComponent
					);
